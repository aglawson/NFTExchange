...
contract Auction {
  ...
  MoraribleToken public mora;
  AuctionState public state;
  bool public rewardClaimed;

  /* CONSTRUCTOR */
  constructor(address _owner, uint _auctionLength, uint _startingPrice, address _moraCollectible) public {
    owner = _owner;
    startingPrice = _startingPrice;
    startTime = block.timestamp;
    endTime = startTime + _auctionLength * 1 seconds;
    mora = MoraribleToken(_moraCollectible);
   ...
   */
  function bid(bytes memory _bidValue) external {
    require(block.timestamp < endTime);  // a user can only bid within the time period
   ...
   */
  function getHighestBidder(address[] memory _bidders, uint[] memory _bidAmounts, uint[] memory _stakeAmounts) public pure returns (address, uint) {
  ...
  function claimReward() external {
    ...
    rewardClaimed = true;
    mora.createItem(msg.sender, endTime);  // mint an ERC721 Enigma Collectible with arbitrary tokenID (just use the end time)
  }
   ...
   */
  modifier onlyEnigma() {
    require(msg.sender == address(mora));
    _;
  }
   ...
   */
  function getBidValueForBidder(address _bidder) public view returns (bytes memory) {
